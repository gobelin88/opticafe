...
Viewer1D::Viewer1D()
{
    ...
    this->rescaleAxes();
    this->xAxis->setLabel(QString("p%1").arg(0));
...
void Viewer1D::setPath(std::vector<VectorXd> path)
{
    this->path=path;

    graph(1)->setData( extract(path,box.id1),buildNULLQVector(path.size()));
    rescaleAxes();
    replot();
}

void Viewer1D::slot_set_data(std::vector<double> data,Box box)
{
    this->box=box;
    this->data=data;
